...
export async function guessHandler(guess : string, word : string): Promise<{ match : boolean , songTitle : string }> {
    ...
    let songTitle = data.response.hits[0].result.title + " - " + data.response.hits[0].result.artist_names;
    ...
    return {match : validGuess, songTitle : songTitle};
}